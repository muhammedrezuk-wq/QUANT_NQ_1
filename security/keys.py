...
class KdfParams:
    ...
    def from_dict(cls, data: dict) -> KdfParams:
        ...
        if algorithm not in ("argon2id", "pbkdf2-sha256"):
            raise ValueError("unsupported KDF algorithm")
        try:
            salt = base64.b64decode(data.get("salt", ""), validate=True)
            ...
            raise ValueError("invalid KDF parameters") from exc
        if not 16 <= len(salt) <= 64: raise ValueError("invalid KDF salt length")
        if algorithm == "pbkdf2-sha256" and not 100_000 <= iterations <= 10_000_000:
            raise ValueError("KDF iterations out of bounds")
        if not 8_192 <= memory_kib <= 262_144: raise ValueError("KDF memory out of bounds")
        if not 1 <= parallelism <= 16: raise ValueError("KDF parallelism out of bounds")
        return cls(algorithm=algorithm, salt=salt, iterations=iterations,
