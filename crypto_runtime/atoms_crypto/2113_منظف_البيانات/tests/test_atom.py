...
root=Path(__file__).resolve().parents[3];folder=Path(__file__).resolve().parents[1];sys.path.insert(0,str(root));spec=importlib.util.spec_from_file_location('_t113',folder/'atom.py');m=importlib.util.module_from_spec(spec);sys.modules['_t113']=m;spec.loader.exec_module(m)
class L:
