...
root=Path(__file__).resolve().parents[4];folder=Path(__file__).resolve().parents[1];sys.path.insert(0,str(root));spec=importlib.util.spec_from_file_location('_t112',folder/'atom.py');m=importlib.util.module_from_spec(spec);sys.modules['_t112']=m;spec.loader.exec_module(m)
class L:
