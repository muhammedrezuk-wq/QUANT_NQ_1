...
def scan(root: Path) -> DiscoveryReport:
    ...
    # Python 3.13 exposes recurse_symlinks; the project still supports 3.12,
    # so keep a portable follow-links fallback. Unified market layouts use
    # directory links for common atoms and must discover them as real atoms.
    try:
        manifest_paths = root.rglob(MANIFEST_FILENAME, recurse_symlinks=True)
    except TypeError:  # Python 3.12
        import os
        manifest_paths = (
            Path(directory) / filename
            for directory, _dirs, files in os.walk(root, followlinks=True)
            for filename in files if filename == MANIFEST_FILENAME
        )
