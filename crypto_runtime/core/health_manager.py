...
class HealthManager:
    ...
    async def _restart(self, atom_id: int, record: AtomRecord) -> None:
        ...
        try:
            await call_lifecycle(current.instance.stop(), "stop")
        except Exception:  # noqa: BLE001 — إيقاف بأفضل جهد فقط (يشمل تجاوز المهلة)
            pass

        try:
            await call_lifecycle(current.instance.start(), "start")
