...
class FileSecretProvider(ISecretProvider):
    ...
    def open(self) -> SecretProviderState:
        ...
        try:
            ...
            _log.error("معاملات اشتقاق المخزن غير صالحة: %s", type(exc).__name__)
            self._state = SecretProviderState.UNAVAILABLE
            return self._state
    ...
    def _read_envelope(self) -> dict | None:
        try:
            if self._path.stat().st_size > 16 * 1024 * 1024:
                raise ValueError("vault envelope too large")
        ...
        if envelope.get("format") != VAULT_FORMAT:
            ...
            return None
        try: version = int(envelope.get("version", 0))
        except (TypeError, ValueError): version = -1
        if version != VAULT_VERSION or envelope.get("cipher") != "fernet":
            _log.error(
                "إصدار/تشفير المخزن غير مدعوم: %s (المدعوم %s)",
