...
async def test_release_baseline_is_loaded_and_verified_on_start():
 with tempfile.TemporaryDirectory() as t:
  ...
  baseline=P(t)/'baseline.json';baseline.write_text(json.dumps({'format':'quant-nq-integrity-baseline','scope_digest':probe._scope_digest,'items':probe._collect()}),encoding='utf8')
