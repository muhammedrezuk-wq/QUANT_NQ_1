...
EVENT_OUT = "stats.variance.state"

METHOD = "rolling_variance_sample"
ID_VARIANCE = "variance"
...
class Atom(AtomBase):
    ...
    async def _emit(self, symbol: str, timeframe: str, cycle_id: str,
        ...
        if self._context is None:
            return
        count = len(window)
        base = {"symbol": symbol, "id": ID_VARIANCE, "cycle_id": cycle_id,
                "timeframe": timeframe}
        ...
        meta["value"] = round(variance, _VALUE_DP)
        await self._context.publish(EVENT_OUT, {
            **window_evidence(have=len(window), need=self._window_size),
