...
EVENT_OUT = "stats.mode.state"

METHOD = "rolling_mode"
ID_MODE = "mode"

SIGNAL_MODE = "mode"
SIGNAL_UNIFORM = "uniform"
...
class Atom(AtomBase):
    ...
    async def _on_tick(self, payload: dict[str, Any]) -> None:
        ...
        window.append(round(price, _VALUE_DP))
        self._ticks_seen += 1
    ...
    async def _emit(self, symbol: str, timeframe: str, cycle_id: str,
        ...
        signal = SIGNAL_MODE if freq > 1 else SIGNAL_UNIFORM
        ...
        meta = {"method": METHOD, "timeframe": timeframe, "window": self._window_size,
                "count": count, "value": value, "frequency": freq}
        await self._context.publish(EVENT_OUT, {
            **window_evidence(have=len(window), need=self._window_size),
            "symbol": symbol, "id": ID_MODE, "cycle_id": cycle_id,
            "timeframe": timeframe,
            "status": STATUS_OK, "signal": signal, "score": 0,
