...
_spec = _ilu.spec_from_file_location(
    "_atom302", _Path(__file__).resolve().parents[1] / "atom.py")
_mod = _ilu.module_from_spec(_spec)
sys.modules["_atom302"] = _mod
...
class FakeEventBus:
    ...
    def make_context(self, config):
        return AtomContext(atom_id=302, config=config, logger=_NullLogger(),
...
async def test_median_odd():
    print("\n--- test_median_odd ---")
    _atom, _bus, out = await _run([30, 10, 20])
    assert out[-1]["metadata"]["value"] == 20, out[-1]["metadata"]["value"]
    print("OK — وسيط فردي [30,10,20] = 20")


async def test_median_even():
    print("\n--- test_median_even ---")
    _atom, _bus, out = await _run([10, 20, 30, 40])
    assert out[-1]["metadata"]["value"] == 25, out[-1]["metadata"]["value"]
    print("OK — وسيط زوجي [10,20,30,40] = 25")
...
async def test_contract_shape_complete():
    ...
    assert "value" in last["metadata"] and last["id"] == "median"
...
async def main():
    tests = [test_median_odd, test_median_even, test_contract_shape_complete,
             test_health_states]
    failed = []
    for t in tests:
        try:
