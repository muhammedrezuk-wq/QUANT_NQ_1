...
_spec = _ilu.spec_from_file_location(
    "_atom308", _Path(__file__).resolve().parents[1] / "atom.py")
_mod = _ilu.module_from_spec(_spec)
sys.modules["_atom308"] = _mod
_spec.loader.exec_module(_mod)
Atom = _mod.Atom
EVENT_OUT = _mod.EVENT_OUT

CFG = {"window_size": 20, "slope_threshold": 0}
...
class FakeEventBus:
    ...
    def make_context(self, config):
        return AtomContext(atom_id=308, config=config, logger=_NullLogger(),
...
async def test_rising_trend():
    print("\n--- test_rising_trend ---")
    _atom, _bus, out = await _run([10, 20, 30, 40])  # خطّ مثالي صاعد slope=10
    last = out[-1]
    assert last["signal"] == "rising", last["signal"]
    assert last["metadata"]["value"] == 10
    print(f"OK — صاعد: rising · slope={last['metadata']['value']}")


async def test_falling_trend():
    print("\n--- test_falling_trend ---")
    _atom, _bus, out = await _run([40, 30, 20, 10])  # slope=-10
    last = out[-1]
    assert last["signal"] == "falling", last["signal"]
    assert last["metadata"]["value"] == -10
    print(f"OK — هابط: falling · slope={last['metadata']['value']}")
...
async def test_contract_shape_complete():
    ...
    for field in ("value", "intercept"):
        assert field in last["metadata"]
    assert last["id"] == "regression"
...
async def main():
    tests = [test_insufficient_one_point, test_rising_trend, test_falling_trend,
