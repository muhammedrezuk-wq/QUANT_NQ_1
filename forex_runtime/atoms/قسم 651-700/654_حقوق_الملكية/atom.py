...
EVENT_IN="platform.account.state";EVENT_OUT="portfolio.equity.state"
class Atom(AtomBase):
 ...
 async def _on(self,p):
  ...
  self._updates+=1;await self._context.publish(EVENT_OUT,{"component":"equity","event_name":EVENT_OUT,"account_id":p.get("account_id"),"equity":p.get("equity"),"currency":p.get("currency"),"measured_at":p.get("measured_at")})
 async def health_check(self):
  if not self._running:return HealthStatus(state=HealthState.UNHEALTHY,message="NOT_STARTED")
  return HealthStatus(state=HealthState.HEALTHY if self._updates else HealthState.DEGRADED,message="equity_updates=%d"%self._updates)
