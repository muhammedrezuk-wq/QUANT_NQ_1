...
EVENT_IN="platform.account.state";EVENT_OUT="portfolio.margin.state"
class Atom(AtomBase):
 ...
 async def _on(self,p):
  ...
  self._updates+=1;await self._context.publish(EVENT_OUT,{"component":"margin","event_name":EVENT_OUT,"account_id":p.get("account_id"),"margin":p.get("margin"),"currency":p.get("currency"),"measured_at":p.get("measured_at")})
 async def health_check(self):
  if not self._running:return HealthStatus(state=HealthState.UNHEALTHY,message="NOT_STARTED")
  return HealthStatus(state=HealthState.HEALTHY if self._updates else HealthState.DEGRADED,message="margin_updates=%d"%self._updates)
