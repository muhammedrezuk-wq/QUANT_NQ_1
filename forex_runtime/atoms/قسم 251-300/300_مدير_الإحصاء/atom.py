...
EVENT_OUT = "stats.cycle.collected"
EVENT_LIVE = "stats.section.live"
# Section 300 closure, atoms-first (owner order 2026-08-23): the units panel --
# every statistics unit declares ITSELF: what it last said, its own rhythm, its
# delivery count and age. Sourceless units are DECLARED excluded, never hidden.
EVENT_PANEL = "stats.units.state"

# Section 300 closure: every statistics unit has a live source -- nothing is
# excluded here. The tuple stays declared (and empty) so the exclusion
# machinery keeps one uniform shape across section managers.
...
_UNIT_IDS = tuple(
    event.replace("stats.", "").replace(".state", "")
...
@section_atom("300", "300")
@section_live("300", EVENT_LIVE)
class Atom(AtomBase):
    def __init__(self) -> None:
        ...
        self._late = 0
        self._invalid = 0
        # Section 300 closure: per-unit self-declared registry.
    ...
    async def initialize(self, context: AtomContext) -> None:
        ...
        # Section 300 closure (owner vision, atoms-first 2026-08-23): same
        ...
        # naturally. All 17 units have live sources -- none is excluded.
    ...
    async def _emit_panel(self, scope: tuple[str, str, str, str],
        ...
        # Section 300 closure: one event, one row per unit. Sourced units
