...
@pytest.mark.asyncio
async def test_failed_upgrade_rolls_back_old_atom(atoms_root: Path) -> None:
    """A replacement that cannot start must reattach the old running instance."""
    ...
    assert registry.get(30).state == AtomState.RUNNING
    assert str(registry.get(30).manifest.version) == "1.0.0"
