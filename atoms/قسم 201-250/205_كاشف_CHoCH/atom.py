...
EVENT_OUT = "structure.choch.state"

METHOD = "close_break_reversal"
ID_CHOCH = "choch"
...
class Atom(AtomBase):
    ...
    async def _on_external(self, payload: dict[str, Any]) -> None:
        ...
        if break_dir is not None:
            self._breaks += 1
            is_choch = st["dir"] is not None and break_dir != st["dir"]
            st["dir"] = break_dir
            if is_choch:
                signal = SIGNAL_CHOCH
                direction = break_dir
    ...
    async def health_check(self) -> HealthStatus:
        ...
        return HealthStatus(
            ...
            message="inputs=%d breaks=%d choch=%d tracked=%d" % (
                self._inputs_seen, self._breaks, self._choch, len(self._state)),
            details={"inputs": self._inputs_seen, "breaks": self._breaks,
                     "choch": self._choch, "emitted": self._emitted,
