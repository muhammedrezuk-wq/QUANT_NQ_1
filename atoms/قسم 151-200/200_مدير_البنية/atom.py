...
# 1.4.0 (2026-08-25): panel emission PACED (once per second per scope; the
...
EVENT_LIVE = "structure.section.live"
# Section 200 closure, atoms-first (owner order 2026-08-23): the units panel --
# every structure unit declares ITSELF: what it last said, its own rhythm, its
# delivery count and age. The manager aggregates what arrived -- it invents
# nothing and waits for nobody.
EVENT_PANEL = "structure.units.state"
...
_UNIT_IDS = tuple(
    event.replace("structure.", "").replace(".state", "")
...
@section_atom("200", "200")
@section_live("200", EVENT_LIVE)
class Atom(AtomBase):
    def __init__(self) -> None:
        ...
        self._late = 0
        self._invalid = 0
        # Section 200 closure: per-unit self-declared registry.
    ...
    async def initialize(self, context: AtomContext) -> None:
        ...
        # Section 200 closure (owner vision, atoms-first 2026-08-23): the
        # manager no longer opens a cycle per tick. Measured before: 11,131
        # tick cycles vs 79 unit deliveries -- every cycle was forwarded empty
        # and 209 rejected all of them, while real unit results landed as
        # silent "late" drops. Now a cycle opens from the FIRST unit delivery
        # under the unit's OWN identity and completes naturally with its
        # siblings -- nobody waits for anybody, and nothing empty is pushed.
    ...
    async def _emit_panel(self, scope: tuple[str, str, str, str],
        ...
        # Section 200 closure: one event, one row per expected unit -- all
        # self-declared by the unit: what it said, its rhythm, its deliveries,
        # its age, and (candle rhythm) its next expected delivery time.
    ...
    async def health_check(self) -> HealthStatus:
        ...
        return HealthStatus(
            ...
            message="opened=%d forwarded=%d open=%d late=%d units=%d panel=%d" % (
