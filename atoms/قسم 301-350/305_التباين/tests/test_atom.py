...
_spec = _ilu.spec_from_file_location(
    "_atom305", _Path(__file__).resolve().parents[1] / "atom.py")
_mod = _ilu.module_from_spec(_spec)
sys.modules["_atom305"] = _mod
...
class FakeEventBus:
    ...
    def make_context(self, config):
        return AtomContext(atom_id=305, config=config, logger=_NullLogger(),
...
async def test_variance_computed():
    print("\n--- test_variance_computed ---")
    _atom, _bus, out = await _run([2, 4, 6])  # sample var = 8/2 = 4
    assert out[-1]["metadata"]["value"] == 4, out[-1]["metadata"]["value"]
    print(f"OK — تباين [2,4,6] = {out[-1]['metadata']['value']} (عيّنة)")
...
async def test_contract_shape_complete():
    ...
    assert "value" in last["metadata"] and last["id"] == "variance"
...
async def main():
    tests = [test_insufficient_one_point, test_variance_computed,
