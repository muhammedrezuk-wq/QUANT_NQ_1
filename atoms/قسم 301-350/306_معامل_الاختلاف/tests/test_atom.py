...
_spec = _ilu.spec_from_file_location(
    "_atom306", _Path(__file__).resolve().parents[1] / "atom.py")
_mod = _ilu.module_from_spec(_spec)
sys.modules["_atom306"] = _mod
...
class FakeEventBus:
    ...
    def make_context(self, config):
        return AtomContext(atom_id=306, config=config, logger=_NullLogger(),
...
async def test_cv_computed():
    print("\n--- test_cv_computed ---")
    _atom, _bus, out = await _run([2, 4, 6])  # mean=4, std=2 → cv=0.5
    assert out[-1]["metadata"]["value"] == 0.5, out[-1]["metadata"]["value"]
    print(f"OK — معامل اختلاف [2,4,6] = {out[-1]['metadata']['value']} (انحراف/متوسط)")
...
async def test_contract_shape_complete():
    ...
    assert "value" in last["metadata"] and last["id"] == "cv"
...
async def main():
    tests = [test_insufficient_one_point, test_cv_computed,
