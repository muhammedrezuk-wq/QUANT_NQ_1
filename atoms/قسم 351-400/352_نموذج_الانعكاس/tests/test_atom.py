...
async def test_live_tick_contract_and_snapshot():
    module, atom, bus = await make_atom(352, manifest_config(352))
    for i in range(70):
        price = 100.0 + (0.02 if i % 2 else -0.02) if i < 69 else 99.0
        await atom._on_tick(validated_tick(i, price=price))
    rows = bus.payloads("probability.reversal.state")
    assert len(rows) == 70
    last = rows[-1]
