...
_spec = _ilu.spec_from_file_location(
    "_atom252", _Path(__file__).resolve().parents[1] / "atom.py")
_mod = _ilu.module_from_spec(_spec)
sys.modules["_atom252"] = _mod
...
class FakeEventBus:
    ...
    def make_context(self, config):
        return AtomContext(atom_id=252, config=config, logger=_NullLogger(),
...
async def test_pool_high_becomes_buyside():
    print("\n--- test_pool_high_becomes_buyside ---")
    _atom, _bus, out = await _run([_pool("pool_high", 12, "high", pt=2, score=67)])
    last = out[-1]
    assert last["signal"] == "buyside", last["signal"]
    assert last["metadata"]["price"] == 12 and last["metadata"]["side"] == "high"
    assert last["confidence"] == 0.67, last["confidence"]
    print(f"OK — بركة قمّة → سيولة شراء: price={last['metadata']['price']} "
...
async def test_confidence_scales_with_pool_score():
    ...
    _atom, _bus, weak = await _run([_pool("pool_high", 12, "high", pt=2, score=20)])
    _atom, _bus, strong = await _run([_pool("pool_high", 12, "high", pt=2, score=90)])
...
async def test_pool_low_ignored():
    print("\n--- test_pool_low_ignored ---")
    _atom, _bus, out = await _run([_pool("pool_low", 8, "low", pt=2)])
    assert out[-1]["signal"] == "none", "بركة قاع ليست سيولة شراء"
    print("OK — بركة قاع تُستبعَد (شغل 253)")
...
async def test_health_states():
    ...
    await atom._on_pool(_pool("pool_high", 12, "high", pt=1))
