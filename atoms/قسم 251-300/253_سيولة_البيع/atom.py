...
EVENT_OUT = "liquidity.sellside.state"
...
class Atom(AtomBase):
    ...
    async def _on_pool(self, payload: dict[str, Any]) -> None:
        ...
        is_sellside = payload.get("signal") == POOL_LOW
        signal = SIGNAL_SELLSIDE if is_sellside else SIGNAL_NONE
        price = meta_in.get("price") if is_sellside else None
        pool_time = meta_in.get("pool_time") if is_sellside else None
        score = int(payload.get("score", 0)) if is_sellside else 0
        confidence = _confidence_from_score(score) if is_sellside else 0.0
        if is_sellside:
            self._pools += 1
        meta = {"method": METHOD, "timeframe": timeframe, "side": SIDE_LOW,
                "price": price, "pool_time": pool_time, "close": close}
        await self._context.publish(EVENT_OUT, {
            "symbol": symbol, "id": ID_SELLSIDE, "cycle_id": cycle_id,
            "timeframe": timeframe,
    ...
    async def health_check(self) -> HealthStatus:
        ...
        return HealthStatus(
            ...
            message="inputs=%d sellside=%d emitted=%d" % (
                self._inputs_seen, self._pools, self._emitted),
            details={"inputs": self._inputs_seen, "sellside": self._pools,
